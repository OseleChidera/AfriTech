...
const DeviceInfoSection = ({ addReview, deviceData, user }) => {
    ...
    useEffect(()=>{
        console.log(`DeviceInfoSection    ${userData}  `)
    },[])
    ...
    return (
        ...
        <div id="img-container" className='w-[98%] p-2 mx-auto border border-black'>
            <div className="flex gap-5 w-fit overflow-x-scroll">
                {
                        deviceData.productImages?.map((imageURL,index) => (
                        <Image alt={`product-image ${index}`} src={imageURL} className='border border-black' width={140} height={140} loading='lazy' />
                    ))
                }
                   
            </div>
        </div>
        <div id="comments" className='w-[98%] p-2 mx-auto border border-black'>
                <h1 className='font-bold text-[#005377] capitalize border border-black w-fit mx-auto mb-2'>product reviews</h1>
                <div id="add-review" className="flex flex-col gap-2 border border-black mb-3">
                    <textarea id="review-input" type="text" value={review} onChange={(e) => setReview(e.target.value)} className='p-2 text-sm' placeholder='Enter your review of the product' maxLength={250}> </textarea>
                    <button className='font-bold text-white capitalize bg-[#005377] rounded-md p-2' onClick={() => {
                        addReview(deviceData.id, review)
                        setReview('')
                    }}>add review</button>
                </div>
                <div id="previous-reviews" className='flex flex-col gap-2'>
                    {
                        deviceData.reviews?.map((review, index) => (<CustomerReview data={deviceData} reviewOBJ={review} user={user} id={index}/>))
                    }
                </div>
        </div>
        </div>
